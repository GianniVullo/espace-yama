<template>
  <nav
    class="fixed top-0 z-20 flex w-screen items-center justify-between bg-accent-200 bg-opacity-25 px-3 py-3 duration-500 hover:bg-opacity-70">
    <ul class="hidden items-center md:flex">
      ...
      <li class="mr-5 last:mr-0">
        <a
          ...
          href="https://docs.google.com/spreadsheets/d/14KV-9y9GQkTSF6dxUeCEvEQLgYC-vBXItyNzKFF_gcs/edit#gid=0"
          >Nos offres</a
        >
      </li>
    </ul>
    <NuxtLink to="/">
        <button>Espace Yama</button>
    </NuxtLink>
    <ul class="hidden items-center md:flex">
      <li class="mr-5 last:mr-0" v-for="(link, idx) in menuItems" :key="idx">
        <NuxtLink
    ...
    <ul
      ...
      <li class="mt-3 cursor-pointer" @click="toggleDark()">
        <svg
          v-if="isDark"
          xmlns="http://www.w3.org/2000/svg"
          class="h-6 fill-primary-400"
          viewBox="0 0 512 512">
